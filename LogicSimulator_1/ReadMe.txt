================================================================================
    MFC 라이브러리 : LogicSimulator_1 프로젝트 개요
===============================================================================

응용 프로그램 마법사에서 이 LogicSimulator_1 응용 프로그램을 만들었습니다. 이 응용 프로그램은 MFC의 기본 사용법을 보여 줄 뿐만 아니라 응용 프로그램을 작성하기 위한 기본 구조를 제공합니다.

LogicSimulator_1 응용 프로그램을 구성하는 각 파일에 대한
요약 설명이 포함되어 있습니다.

LogicSimulator_1.vcxproj
...
LogicSimulator_1.vcxproj.filters
...
LogicSimulator_1.h
    ...
    여기에는 resource.h를 비롯한 다른 프로젝트별 헤더가 포함되어 있으며 CLogicSimulator_1App 응용 프로그램 클래스가 선언되어 있습니다.

LogicSimulator_1.cpp
    이는 응용 프로그램 클래스 CLogicSimulator_1App가 포함된 기본 응용 프로그램 소스 파일입니다.
...
res\LogicSimulator_1.ico
    아이콘 파일이며, 응용 프로그램의 아이콘으로 사용됩니다. 이 아이콘은 기본 리소스 파일인 LogicSimulator_1.rc에 의해 포함됩니다.

res\LogicSimulator_1.rc2
    이 파일에는 Microsoft Visual C++ 이외의 다른 도구에서 편집한 리소스가 포함되어 있습니다. 리소스 편집기로 편집할 수 없는 모든 리소스는 이 파일에 넣어야 합니다.

LogicSimulator_1.reg
...
res\Toolbar.bmp
    ...
    초기 도구 모음과 상태 표시줄은 CMainFrame 클래스에서 생성됩니다. 이 도구 모음 비트맵을 리소스 편집기로 편집하고, LogicSimulator_1.rc의 IDR_MAINFRAME TOOLBAR 배열을 업데이트하여 도구 모음 단추를 추가합니다.
/////////////////////////////////////////////////////////////////////////////

응용 프로그램 마법사에서 문서 형식과 뷰를 한 개씩 만듭니다.

LogicSimulator_1Doc.h, LogicSimulator_1Doc.cpp - 문서
    이 파일에는 CLogicSimulator_1Doc 클래스가 포함됩니다. 이 파일을 편집하여 특수 문서 데이터를 추가하고 CLogicSimulator_1Doc::Serialize를 통해 파일 저장 및 로드를 구현합니다.
    문서에는 다음과 같은 문자열이 있습니다.
        파일 확장명:      ls
        파일 형식 ID:        LogicSimulator1.Document
        주 프레임 캡션:  LogicSimulator_1
        Doc 형식 이름:       LogicSimulator_
        필터 이름:         LogicSimulator_1 Files (*.ls)
        파일의 새 약식 이름: LogicSimulator_
        파일 형식의 긴 이름: LogicSimulator_1.Document

LogicSimulator_1View.h, LogicSimulator_1View.cpp - 문서 뷰
    이 파일에는 CLogicSimulator_1View 클래스가 포함됩니다.
    CLogicSimulator_1View 개체는 CLogicSimulator_1Doc 개체를 보는 데 사용됩니다.
...
StdAfx.h, StdAfx.cpp
    이 파일은 미리 컴파일된 헤더(PCH) 파일 LogicSimulator_1.pch와 미리 컴파일된 형식(PCT) 파일 StdAfx.obj를 빌드하는 데 사용됩니다.

Resource.h
    새 리소스 ID를 정의하는 표준 헤더 파일입니다. Microsoft Visual C++에서 이 파일을 읽고 업데이트합니다.

LogicSimulator_1.manifest
